...
repo_path = os.path.join(
    project_root, "churn_feature_store", "churn-feature", "feature_repo"
)
...
FEATURES = [
    ...
    "customer_demographics:subscription_type_premium",
    "customer_demographics:contract_length_monthly",
    # ---------- customer_behavior ----------
...
def get_customer_features(
    customer_id: Union[int, str, List[Union[int, str]]],
) -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    entity_rows = [{"customer_id": str(i)} for i in range(2, 40)]
    store = FeatureStore(repo_path=repo_path)
    df = store.get_online_features(
        entity_rows=entity_rows,
        features=FEATURES,
